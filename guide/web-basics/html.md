...
HTML（超文本标记语言）是构建 Web 内容的基础语言，构成了任何网站的“骨架”。它定义了网站的整体结构和内容元素，例如按钮、图像、文本和列表。每个 HTML 元素由一个起始标签、内容和一个结束标签组成。文档结构包括用于元数据（标题、CSS 链接、网站图标、作者信息）的 `<head>` 部分和用于可见内容的 `<body>` 部分。HTML 元素还可以通过属性提供附加信息。

将 HTML 比作“骨架”，突显了其基础且不可替代的作用。如果 HTML 文档结构不佳，样式 (CSS) 和交互 (JavaScript) 将无法正常工作，这确立了清晰的因果依赖关系。

## **II.2.1 文档结构与元数据**

一个完整的 HTML 文档通常从 `<!doctype html>` 开始，随后是 `<html>`、`<head>` 和 `<body>`。`<head>` 中的信息不会直接作为页面正文显示，却会深刻影响浏览器解析、搜索引擎理解、社交分享和移动端体验。

常见元数据包括：

- **字符集**：`<meta charset="utf-8">` 确保中文、符号和多语言内容被正确解析。
...
元数据是页面和外部环境沟通的接口。一个结构良好的 `<head>` 能让浏览器、搜索引擎、辅助技术和社交平台更准确地理解页面。
...
对语义化 HTML 的强调，表明了从纯粹的展示性标记向注重机器可读性意义的转变。这直接影响了可访问性工具（例如屏幕阅读器依赖结构）和 SEO（搜索引擎理解内容上下文），体现了良好 HTML 实践与更广泛的 Web 性能和包容性之间的因果关系。

## **II.2.3 链接、媒体与表格**

HTML 不只是堆叠文本，它还负责把内容组织成可导航、可引用、可感知的信息结构。
...
`<input>`、`<button>` 等元素在表单中使用。表单是用户交互的主要入口，直接将 HTML 的结构作用与 JavaScript 的动态能力和后端处理联系起来。
...
- 使用 HTML validator、Lighthouse 或浏览器可访问性面板发现遗漏属性和结构问题。

HTML 的目标是提供稳定、语义清晰、可被机器和人共同理解的文档结构。
