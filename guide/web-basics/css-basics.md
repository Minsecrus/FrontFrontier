...
理解层叠、特异性和继承至关重要，因为 CSS 本质上是声明式的和非线性的。对这些概念的误解，直接导致不可预测的样式行为和”样式覆盖问题”，凸显了理论理解与实际可维护性之间的因果关系。
...
从旧的布局方法（浮动、表格）到 Flexbox 和 Grid 的转变，代表了 CSS 的重大演变。这些现代技术简化了复杂布局，是响应式设计的基础，直接影响开发效率和 UI 质量。
...
- **表单样式**：输入框、选择器、复选框、单选框、按钮和错误提示应保持清晰的 focus、hover、disabled、invalid 状态。自定义样式应保留键盘焦点，并用文本、颜色和图标共同表达错误。
...
它们越来越多地进入 **Baseline / Interop** 语境，开始成为现代 CSS 与平台级交互设计的现实选项。  
学习这些能力时，重点是判断它们何时可以在生产项目里作为正式能力使用。
...
- **calc()** 允许数学表达式，而 **clamp()** 将值限制在上下限之间。这些高级单位和函数使开发者能够创建高度动态和适应性强的设计，无需过度依赖 JavaScript 进行布局，从而提高性能和可维护性。
...
- **Element - 元素 ( `__element` )**
  - **定义：** 块的组成部分，它在语义上从属于这个块，意义依附于块的上下文。如果“产品卡片”是块，那么“卡片标题”、“卡片图片”和“购买按钮”就是元素。
