...
- **表单状态**：介于 UI 状态和业务数据之间，包含输入值、校验错误、脏状态、提交中状态和服务端返回错误。  
  简单表单可以直接交给框架管理，复杂表单则需要专门的表单库或清晰的状态约定。

服务器状态和客户端状态最大的区别在于：**服务器状态不由前端拥有，前端只是它的一份缓存视图**。  
因此，服务器状态管理关注的是“这份数据是否新鲜、何时重新获取、更新失败如何回滚”，缓存策略、同步策略和错误恢复策略需要共同配合。

- **缓存**：减少重复请求，提高页面切换和回退时的响应速度。
- **失效**：在数据被修改后，让相关查询重新变为陈旧状态，触发重新获取。
- **后台刷新**：先展示已有数据，再在后台请求新数据，减少空白加载。
- **乐观更新**：先在界面上表现为成功，再在请求失败时回滚或提示用户。
- **请求去重**：多个组件需要同一份数据时，合并相同请求。
- **错误恢复**：请求失败时提供重试、降级、回退数据或明确的用户反馈。

在 React 生态中，[TanStack Query](https://tanstack.com/query/latest) 和 SWR 就是围绕服务器状态设计的工具。  
...
这样拆分后，每份状态都交给最了解其生命周期的系统。判断时依次问：它能否从 props 或其他状态计算出来？刷新后是否应该保留？是否应该出现在 URL？真正的数据所有者是谁？如果数据由浏览器拥有、且被多个相距较远的组件共同修改，就适合放进客户端共享 store。

清晰区分这些状态，是现代前端架构中非常基础但又常被忽略的一步。  
这直接影响组件边界、数据获取位置、缓存策略、用户体验和代码可维护性。解决很多状态管理问题，都要先判断这份状态到底属于哪里。
