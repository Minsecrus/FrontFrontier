...
长久以来，JavaScript 几乎是 Web 世界唯一的原生编程语言，承载了浏览器中几乎所有的逻辑与交互。尽管现代 JavaScript 引擎（如 [V8](https://v8.dev/)）通过即时编译（JIT）等技术大幅提升了性能，但在执行计算密集型任务（如 3D 图形渲染、视频编解码、复杂科学计算）时，其动态语言的本质仍然与原生语言（如 C++、Rust）存在难以逾越的性能鸿沟。为了打破这一天花板，**WebAssembly ([WASM](https://webassembly.org/))** 应运而生。
...
1. **极致的性能 (Near-Native Performance)**
   WASM 的核心吸引力在于其卓越的性能。作为一种预编译的、静态类型的低级二进制格式，浏览器可以极快地完成验证和机器码编译，跳过了 JavaScript 所需的复杂动态解析与优化过程。这使得 WASM 在处理 CPU 密集型任务时能够发挥接近原生代码的执行效率，为在浏览器中运行重度应用（如专业级图像/视频编辑器、大型游戏、CAD 软件）打开了大门。
...
3. **可预测的性能与安全性 (Predictable Performance & Security)**
   相较于 JavaScript，WASM 的执行性能更加稳定和可预测，因为它采用提前编译后的低级字节码模型。同时，WASM 运行在一个与 JavaScript 环境隔离的、内存安全的**沙箱 (Sandbox)** 中。它默认无法直接访问 DOM 或任意 Web API，所有与外部世界的交互都必须通过明确的 JavaScript API 作为中介。这种设计确保了 WASM 模块的执行是高度安全的，不会对用户系统造成威胁。
